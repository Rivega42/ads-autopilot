 ...
 * у которого нет ни одной подходящей кампании, — то есть проверяет проводку
 * команды, а не запись. Саму запись из команды — ставки в кабинете, строки
 * ChangeLog, карточки в чате — проходит `cli-optimize-apply.e2e.ts`: там моки
 * площадок подняты внутри процесса команды.
 */
