...
- `pnpm yd:check` — диагностика доступа к API Директа: валидность токена,
  аккаунт, список существующих кампаний, остаток баллов
- Креативы для РСЯ: 15 изображений (5 сюжетов × 1:1, 16:9, 3:4), собираются
  из HTML скриптом `render.mjs`; `strip-bg.py` снимает залитую подложку
  с исходников, приехавших с сайта без альфа-канала
- `pnpm campaign:deploy` — заливка blueprint в Директ через API: campaigns.add →
  adgroups.add → keywords.add → ads.add. По умолчанию холостой прогон,
  реальная отправка только по `--apply`, кампании создаются остановленными
- Резолвер регионов через `dictionaries.get`: имена из blueprint → ID Директа,
  с явной ошибкой на ненайденные вместо молчаливого пропуска
- `docs/campaigns/smartsay/metrika/` — готовый код счётчика и целей для SPA
  клиента: хит на смену маршрута, типизированные цели, точки вызова
