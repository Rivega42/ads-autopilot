...
```json
{
  "error": {
    "error_code": 152,
    "error_string": "Некорректный формат запроса",
    "error_detail": "..."
  }
}
```
...
**Маркировка (ОРД) — обязательна для РФ с 01.09.2022, но кода не требует:**

- **Для прямого рекламодателя площадка маркирует сама.** VK Реклама и Яндекс
  Директ присваивают `erid` каждому креативу и передают данные со статистикой
  в ЕРИР через собственные ОРД. Генерировать токен в коде не нужно.
- От рекламодателя требуется настройка в кабинете, а не разработка: включить
  маркировку на уровне аккаунта, выбрать ОРД, заполнить данные о себе
  (в Директе — «рекламный паспорт»; без них кампании могут не пройти проверку).
- `OrdPartnerPad/ActStat` нужны **только посреднику**: по закону отчитывается
  каждое звено цепочки, и площадка не знает о договоре между сервисом и его
  клиентом. Тогда в ЕРИР передаются контрагенты, договоры и ежемесячные акты
  о расходах в течение 30 дней.
- **Текущая модель — прямой рекламодатель на собственных кабинетах**, поэтому
  в MVP по ОРД задач нет. Если сервис начнёт вести чужие кабинеты по договору,
  отчётность по цепочке придётся добавить.
