 ...
 *
 * Это интерфейс, а не клиент: метода `KeywordsResearch.hasSearchVolume` в
 * `src/clients/yandex-direct/` сегодня нет (там реализованы Campaigns/AdGroups/Ads/
 * Keywords.get, Reports и записи ставок и минус-фраз), а дописывать чужой модуль
 * из семантики нельзя. Поэтому ядро работает против контракта, а реализация
 * подставляется снаружи — когда метод в клиенте появится, здесь ничего не меняется.
 *
