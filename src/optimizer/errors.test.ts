...
describe('FAILURE_ROWS_PER_BATCH_CAP', () => {
  it('строго выше порога тревоги — иначе массовый отказ одной кампании никого не разбудит', () => {
    // `detectAlerts` срабатывает на «больше порога», поэтому потолка, равного
    // порогу, недостаточно: нужен хотя бы один отказ сверху.
    //
    // Проверяется здесь ровно одно — что услышат самого флудящего. Тревога
    // соседнего клиента от этих двух чисел не зависит: её ломало не соотношение
    // констант, а то, что скан выбирал строки одной выборкой на всех. Это
    // свойство проверяется прогоном в `reporter/alerts.test.ts`.
