 ...
 *
 * Поэтому такие зоны сами ничего не доказывают: адресом токен становится, только
 * если рядом есть настоящий признак адреса — схема, `www.` или путь. Их проверяют
 * выше по `looksLikeWebAddress`, и ссылка `https://дети.онлайн` или
 * `дети.онлайн/kursy` по-прежнему читается как ссылка. Голое `дети.онлайн` —
 * нет, и это осознанный размен: клиент с сайтом в такой зоне (их на всю страну
 * тысячи) уедет к человеку через `unconfirmed-landing`, а клиент без сайта не
 * услышит от системы неправду и не заплатит ходом модели за каждое название города.
 */
...
function looksLikeWebAddress(token: string): boolean {
  ...
  // решение — а зона-слово столько не весит (см. `WORD_TLDS`).
