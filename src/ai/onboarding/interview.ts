...
async function advance(ctx: AdvanceContext): Promise<InterviewStep> {
  ...
  // другому — «разберётся человек». Признак поэтому мягче, чем у ступеней записи,
  // где модель что-то поправила (`mentionsWebAddress` против
  // `extractWebAddresses`): цена лишнего «сайта нет» — потерянный клиент в паузе
  // без выхода, цена лишней записи — чужой сайт в объявлении, и второе дороже.
