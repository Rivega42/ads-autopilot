 ...
 *
 * Отказ по паре «вид действия × канал» (VK не умеет минус-слов, Директ — паузы
 * групп) здесь не проверяется намеренно: он обязан случиться до человека, и его
 * делает `assertActionExecutable` при выпуске карточки. Здесь же остаётся честный
 * ответ адаптера — по нему `src/approval/supported.test.ts` и сверяет объявление,
 * а сверять таблицу с самой собой смысла нет.
 */
