...
function clientOf(transport: FakeTransport, extra: Partial<{ unitsReserve: number; ledger: UnitsLedgerWriter; credentials: YandexCredentials }> = {}) {
...
describe('units accounting', () => {
  it('persists every parsed Units header to the ledger', async () => {
    ...
    expect(ledger.entries).toEqual([
      { clientId: 'client-1', method: 'campaigns.get', spent: 15, remaining: 9000, dailyLimit: 64000 },
    ]);
  ...
  it('survives a ledger write failure without losing the API response', async () => {
    ...
    const transport = transportOf([{ headers: { units: '1/2/3' }, data: { result: { ok: true } } }]);
