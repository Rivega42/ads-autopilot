...
describe('dry run', () => {
  const cases: Array<[string, (a: YandexDirectAdapter, ctx: ChannelContext) => Promise<unknown>]> = [
    ...
    [
      'updateAdText',
      (a, c) => a.updateAdText(c, '42', { title: 'Заголовок', text: 'Текст' }),
    ],
  ];
...
describe('writes when dry run is off', () => {
  ...
  it('reads the current negative keywords before replacing them', async () => {
    const transport = transportOf([
      { data: { result: { Campaigns: [{ Id: 5, Name: 'C', NegativeKeywords: { Items: ['даром'] } }] } } },
...
describe('reads', () => {
  ...
  it('converts keyword bids out of micro-units', async () => {
    const transport = transportOf([
      { data: { result: { Keywords: [{ Id: 3, AdGroupId: 7, Keyword: 'купить слона', Bid: 25_000_000, State: 'ON' }] } } },
    ]);
