...
import { getAdGroups, getAds, getCampaigns, getKeywords, getSelfClient } from '@/clients/yandex/entities.js';
import {
  fetchReport,
  reportNumber,
...
function pickAdText(ad: YandexAd): { Title?: string; Title2?: string; Text?: string; Href?: string | null } {
...
function applied(plan: Record<string, unknown>, summary: ActionSummary): WriteResult<ActionSummary> {
...
export class YandexDirectAdapter implements ChannelAdapter {
  ...
  async setBudgets(ctx: ChannelContext, changes: BudgetChange[]): Promise<WriteResult> {
    ...
    const plan = { action: 'Campaigns.update', field: 'DailyBudget', count: updates.length, updates };
    if (ctx.dryRun) return planned(plan);
  ...
  async addNegativeKeywords(
    ...
    const plan = { action: 'Campaigns.update', field: 'NegativeKeywords', campaignExternalId, phrases };
    if (ctx.dryRun) return planned(plan);
  ...
  private requireSuspendable(level: StatLevel): 'campaigns' | 'ads' | 'keywords' {
    ...
    if (!service) {
      ...
      throw new ChannelError(YANDEX_CHANNEL, `Yandex Direct cannot suspend/resume level "${level}"`, {
